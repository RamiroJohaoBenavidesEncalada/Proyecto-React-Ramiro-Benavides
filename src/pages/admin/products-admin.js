...
export const ProductoCreate = () => {
    ...
    const { nombre, precio, observaciones, caducidad, marcaId,tipoProductoId } = producto;
    ...
    return loading ? <div>Loading data...</div> :
        <form className="row" onSubmit={(e) => handleSubmit(e)}>
            <div className="col-6">
                ...
                <div className="input-group has-validation">
                    <input type="text" className="form-control" id="nombre"
                        value={nombre} onChange={e => handleChange(e)} required maxLength="15" />
            ...
            <div className="col-6">
                ...
                <div className="input-group has-validation">
                    <input type="text" className="form-control" id="precio"
                        value={precio} onChange={e => handleChange(e)} required maxLength="10" />
                    <div className="invalid-feedback">
            ...
            <div className="col-6">
                <label htmlFor="caducidad" className="form-label">Caducidad Ej: (2022-12-01T23:44:20.592)</label>
                <div className="input-group has-validation">
                    <input type="text" className="form-control" id="caducidad"
                        value={caducidad} onChange={e => handleChange(e)} required maxLength="80" />
                    <div className="invalid-feedback">
                    Caducidad es obligatorio
                    </div>
                </div>
            </div>
            ...
            <div className="col-6">
                ...
                <select className="form-select" id="marcaId" value={marcaId} required
                    ...
                    {marca.map((cat) => <option key={cat.id} value={cat.id} >{cat.nombre}</option>)}
                </select>
                <div className="invalid-feedback">
                    Marca es requerido
                </div>
            ...
            <div className="col-6">
                <label htmlFor="tipoProductoId" className="form-label">TipoProducto</label>
                <select className="form-select" id="tipoProductoId" value={tipoProductoId} required
                    onChange={e => handleChange(e)}>
                    <option disabled value="">Seleccionar TipoProducto</option>
                    {tipoProducto.map((cat) => <option key={cat.id} value={cat.id} >{cat.nombre}</option>)}
                </select>
                <div className="invalid-feedback">
                    TipoProducto es requerido
                </div>
            </div>

            <div className="col-12 mt-3">
                <button className="btn btn-secondary" type="button" onClick={(e) => navegacion(`/admin/products`)}>Cancelar</button>
    ...
    return loading ? <div>Loading data...</div> :
        <form className="row" onSubmit={(e) => handleSubmit(e)}>
            ...
            <div className="col-6">
                ...
                <div className="input-group has-validation">
                    <input type="text" className="form-control" id="precio"
                        value={precio} onChange={e => handleChange(e)} required maxLength="80" />
                    <div className="invalid-feedback">
                        Precio es obligatorio
                    </div>
                </div>
...
export const ProductoEliminar = () => {
    ...
    const handleSubmit = (event) => {
        ...
        if (event.target.checkValidity() === true) {
            //console.log("Enviar...");
            //console.log(client);
            clientHttp.delete(`/Producto/?marcaId=${productoId}`, producto)
                .then((response) => {
                    navegacion(`/admin/products`)
                });
        }
...
const ProductsList = () => {
    ...
    return <table className="table">
        <thead>
            <td>
                <button onClick={(e) => handlerCrear()}>Crear Productos</button>
            </td>
            <tr>
                <th >Id</th>
                <th >Nombre</th>
